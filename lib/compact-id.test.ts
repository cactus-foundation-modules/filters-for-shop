...
// An id that comes back one character wrong is a card that never re-dresses, or
// a fetch for a product that does not exist - nothing a test of the grid would
...
describe('compactId and expandId', () => {
  ...
  it('writes exactly the standard url-safe base64 of the uuid bytes', () => {
    // Our own spelling, but a standard one, so anybody reading a payload by hand
    // can decode it with the tools they already have.
